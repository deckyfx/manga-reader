"""
Fast Fourier Convolution — FFCResNetGenerator for LaMa Inpainting

Vendored from saicinpainting/training/modules/ffc.py (Er0mangaInpaint).
Original: https://github.com/pkumivision/FFC/blob/main/model_zoo/ffc.py
Paper: https://proceedings.neurips.cc/paper/2020/file/2fd5d41ec6cfab47e32164d5624269b1-Paper.pdf

Only the generator is included (no discriminator). Dependencies inlined.
"""
...
def get_activation(kind="tanh"):
    """Activation factory (inlined from saicinpainting.training.modules.base)."""
    if kind == "tanh":
        return nn.Tanh()
    if kind == "sigmoid":
        return nn.Sigmoid()
...
class FFCSE_block(nn.Module):
    """Squeeze-and-Excitation block for FFC."""
...
class FourierUnit(nn.Module):
    ...
    def __init__(
        ...
        self.use_se = use_se
        # SE layer omitted — not used by our pretrained model
...
class SpectralTransform(nn.Module):
    """Spectral domain transform with optional LFU (Local Fourier Unit)."""
...
class FFC(nn.Module):
    """Fast Fourier Convolution — splits channels into local + global branches."""
...
class FFCResnetBlock(nn.Module):
    """Residual block using FFC layers."""

    def __init__(
        self, dim, padding_type, norm_layer, activation_layer=nn.ReLU,
...
class ConcatTupleLayer(nn.Module):
    """Concatenates local + global branches back into single tensor."""
...
class FFCResNetGenerator(nn.Module):
    """
    FFCResNet Generator for image inpainting.

    Architecture: ReflectionPad → FFC downsample → FFC ResBlocks → Upsample → Conv → Activation

    Args:
        input_nc: Input channels (4 for image + mask)
        output_nc: Output channels (3 for RGB)
        ngf: Base feature count (default 64)
        n_downsampling: Number of downsampling layers (default 3)
        n_blocks: Number of residual blocks (default 9)
        add_out_act: Output activation ('sigmoid', 'tanh', True=tanh, False=none)
        init_conv_kwargs: kwargs for initial conv (ratio_gin, ratio_gout, enable_lfu)
        downsample_conv_kwargs: kwargs for downsample convs
        resnet_conv_kwargs: kwargs for resnet block convs
    """
    ...
    def __init__(
        ...
        for i in range(n_blocks):
            cur_resblock = FFCResnetBlock(
                feats_num_bottleneck, padding_type=padding_type,
