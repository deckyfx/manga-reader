...
import { createUser, deleteUser, listUsers, updateUser, type AccountSummary, type UserRole } from "../../api";
...
/** Everyone on this server: their role, whether they're suspended, and their password when it has to be reset. */
export function UsersSection({ myId }: { myId: number }) {
    ...
    <div>
          ...
          {users.map((user) => (
            <li key={user.id} className="flex flex-wrap items-center gap-2 bg-gray-950 px-3 py-2">
              ...
              <select
                ...
                className={`ml-auto ${fieldClass} py-1 text-xs`}
              >
      ...
      >
        <label className="min-w-36 flex-1 space-y-1">
          ...
          <input value={username} onChange={(e) => setUsername(e.target.value)} placeholder="letterer" className={`w-full ${fieldClass}`} />
        </label>
        <label className="min-w-36 flex-1 space-y-1">
        ...
        <label className="space-y-1">
          ...
          <select value={role} onChange={(e) => setRole(e.target.value as UserRole)} className={`${fieldClass} block`}>
            {ROLES.map((option) => <option key={option} value={option}>{option}</option>)}
          </select>
        </label>
