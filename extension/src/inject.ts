...
// A gallery resolves several pages of one tab at once (see import-queue.ts), and each calls this before its first
// message; without this, they'd all pass the __socrLoaded check before the first injection finishes and each would
// insert the CSS and script again. One in-flight injection per tab, shared by whoever asks while it's running.
...
export async function ensureContentScript(tabId: number): Promise<void> {
  ...
  const task = injectIfMissing(tabId).finally(() => inFlight.delete(tabId));
