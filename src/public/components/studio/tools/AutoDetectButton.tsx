...
export function AutoDetectButton() {
  ...
  const handleAutoDetect = async () => {
    ...
    const [error] = await catchError((async () => {
      ...
      if (!bgImage) {
        showSnackbar("No background image found", "error");
        return;
      }
      ...
      if (!ctx) {
        showSnackbar("Failed to create canvas context", "error");
        return;
      }
      ...
      if (!base64) {
        showSnackbar("Failed to extract image data", "error");
        return;
      }
      ...
      if (!response.data || !response.data.success) {
        ...
        showSnackbar(`Prediction failed: ${errorMsg}`, "error");
        return;
      }

      const regions = response.data.regions;
      if (!regions || regions.length === 0) {
        showSnackbar("No text regions detected", "warning");
        return;
      }
      ...
      showSnackbar(`Created ${createdCount} regions from ${regions.length} detections`, "success");
    })());
