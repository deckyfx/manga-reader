...
3. **Pre-built Base Image**: `comic-reader-base` has manga-ocr + torch → one-time download
...
docker build -t comic-reader:prod .
docker run -d -p 3000:3000 comic-reader:prod
```
...
# View logs
docker logs -f comic-reader

# Stop the container
docker stop comic-reader
docker rm comic-reader
```
...
docker exec comic-reader tesseract --version

# List available languages
docker exec comic-reader tesseract --list-langs
```
...
```bash
docker exec -it comic-reader /bin/bash
```
...
```bash
# Create a test image
docker exec -it comic-reader tesseract \
...
2. In Nginx Proxy Manager, create a proxy host:
   - Domain: `comic-reader.yourdomain.com`
   - Forward Hostname/IP: `comic-reader` (container name)
   - Forward Port: `3000`
...
```bash
# Verify installation
docker exec comic-reader which tesseract
docker exec comic-reader tesseract --version
```
...
docker exec -it comic-reader apt-get update
docker exec -it comic-reader apt-get install tesseract-ocr-jpn
```
